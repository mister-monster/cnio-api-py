...
class cnio():
    ...
    def min_amount(self, send, recieve):
        ...
        endpoint = str("min-amount/" + send + "_" + recieve)
        endpoint = str(self.api + endpoint)
        return requests.get(endpoint).content

    def est_exchange_rate(self, send_amount, send, recieve):
        ...
        endpoint = str("exchange-amount/" + str(send_amount) + "/" + send + "_" + recieve)
        endpoint = str(self.api + endpoint)
        return requests.get(endpoint).content
    ...
    def create_transaction(self, send_amount, send, recieve, addr, extra_id=""):
    ...
    def list_transactions(self, send="", recieve="", status="", limit="", offset=""):
        ...
        query = str("?from=" + send + "&to=" + recieve + "&status=" + status + "&limit=" + limit + "&offset=" + offset)
        endpoint = str("transactions/" + self.key)
        endpoint = str(self.api + endpoint + query)
        return requests.get(endpoint).content
    ...
    def live_tx_updates_socketio(self):
        # this method interfaces with the sockets.io websockets endpoint and gets live transaction updates
        z = 1
